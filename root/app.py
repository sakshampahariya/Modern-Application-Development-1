...
import requests
# from quiz_api import fetch_trivia_questions
import matplotlib
matplotlib.use('Agg')
...
def fetch_trivia_questions(amount=10, category=None, difficulty=None):
    ...
    - category: Optional category ID (see Open Trivia DB documentation)
    - difficulty: Optional difficulty level ('easy', 'medium', 'hard')
    
    Returns:
    - List of formatted questions or None if API call fails
    """
    ...
    params = {
        "amount": amount,
        "type": "multiple"  # We'll use multiple choice questions
    }
    ...
    try:
        ...
        if data["response_code"] == 0:
            # Format questions to match your application's structure
            formatted_questions = []
            for q in data["results"]:
                # Combine correct answer with incorrect answers
                options = q["incorrect_answers"] + [q["correct_answer"]]
                # Find the index of the correct answer (will be 1-based in your app)
                correct_option_index = len(q["incorrect_answers"])
                ...
                formatted_question = {
                    ...
                    "correct_option": correct_option_index + 1  # 1-based indexing
                }
...
@app.route('/admindash/quiz/new/<int:chapter_id>', methods=['GET', 'POST'])
@login_required
...
@app.route('/admindash/quiz/new_api', methods=['GET', 'POST'])
@login_required
def new_api_quiz():
    if not current_user.is_admin:
        flash('Unauthorized access.', 'danger')
    ...
    if request.method == 'POST':
        ...
        # Get API parameters
        question_amount = int(request.form['question_amount'])
        category = request.form.get('api_category') or None
        difficulty = request.form.get('api_difficulty') or None
        ...
        if api_questions:
            ...
            db.session.commit()
            flash("Quiz created successfully with API questions!", "success")
        else:
            flash("Failed to fetch questions from API. Please try again.", "danger")
            
        return redirect(url_for('admindash'))
    
    return render_template('new_api_quiz.html', chapters=chapters)
